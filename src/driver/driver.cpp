...
#include "ara/planning/planner.h"
#include "ara/relational/parsers.h"
...
void Driver::setThreadsPerPipeline(size_t threads) {
  ARA_ASSERT(threads, "Threads per pipeline couldn't be 0");

  option.threads_per_pipeline = threads;
}

void Driver::setMemoryResourceSizePerThread(size_t size) {
  ARA_ASSERT(size, "Memory resource size per thread couldn't be 0");
...
void Driver::setBucketAggregateBuckets(size_t buckets) {
  ARA_ASSERT(buckets, "Buckets of bucket aggregate couldn't be 0");

  option.bucket_aggregate_buckets = buckets;
}

bool Driver::hasNextPipeline() const {
  ARA_ASSERT(executor, "No plan compiled in driver");
